  ...
  public class TempControlBean
  {
    ...
    private void controlTemps()
    {
      ...
      for (Sensors sensor : Sensors.values()) {
        ...
        {
          this.logger.error("Ошибка получения температуры с датчика: " + ex);
          ex.printStackTrace();
        }
      ...
      if ((i > 0) || (this.firstStart))
      {
        ...
        {
          this.logger.error("Ошибка включения или отключения котла: " + ex);
          ex.printStackTrace();
        }
        if (this.firstStart) {
    ...
    private boolean analiseTemp(Sensors sensor)
      throws OwfsException, IOException
    {
      boolean rv = false;
      MaxTemp maxTemp = (MaxTemp)HibernateUtil.getEm().find(MaxTemp.class, sensor);
    ...
    private void analiseBoilerState()
      throws IOException
    {
    ...
    private void colseOrOpen(List<GpioPins> list, boolean state)
    {
      if (state) {
        for (GpioPins gpioPin : list) {
          ...
          {
            this.logger.error("Ошибка закрытия крана " + gpioPin.getName() + ": " + ex);
          }
        }
      } else {
        for (GpioPins gpioPin : list) {
          ...
          {
            this.logger.error("Ошибка открытия крана " + gpioPin.getName() + ": " + ex);
          }
        }
      }
    }
...
/* Location:           E:\classes\
 * Qualified Name:     smarthome.TempControlBean
