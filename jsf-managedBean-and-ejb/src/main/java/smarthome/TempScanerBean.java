...
public class TempScanerBean {
		...
		try {
			...
			try {
				OwfsConnectionConfig connectionConfig = new OwfsConnectionConfig(
						"127.0.0.1", OwfsServerStarter.owsfPort.intValue());
				connectionConfig.setPersistence(OwPersistence.ON);
				connectionConfig
						.setTemperatureScale(OwTemperatureScale.CELSIUS);
			...
			this.logger.error("Ошибка запуска сервера OWFS на порту "
					+ OwfsServerStarter.owsfPort, ex);
		}
	}
	...
	private void scanTemps() {
		...
		for (Sensors sensor : Sensors.values()) {
			...
			try {
				...
				this.logger.error("Ошибка чтения температуры с датчика № "
						+ sensor.getId());
			} catch (Exception e) {
				e.printStackTrace();
				this.logger.error("Ошибка сохранения температуры с датчика № "
						+ sensor.getId());
	...
	public static Float getTemp(Sensors sensor) throws OwfsException {
		if (client != null) {
			if (sensor.getId().length() < 1) {
				throw new OwfsException("Отсутствует идентификатор датчика.",
						-1);
			}
			try {
				return Float
						.valueOf(client.read(
								"/" + sensor.getId() + "/temperature").replace(
								" ", ""));
			} catch (IOException ex) {
