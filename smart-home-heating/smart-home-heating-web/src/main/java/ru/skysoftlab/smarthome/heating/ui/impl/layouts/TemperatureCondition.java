 ...
 */
@UIScoped
@DashBoardElementQualifier(view = HeatingMenuNames.STATISTIC, name = "temperature", order = 2)
	...
	public void reload() {
		...
		if (sensors.size() > 0) {
			...
			for (TemperatureDto tempDto : sensors) {
				try {
					content.addComponent(new Label(tempDto.getSensorName()), 0, y);
					content.addComponent(new Label(tempDto.getTemp().toString()), 1, y);
					y++;
